...
#### Generera deploy-nyckel (engångssetup)

Görs en gång per kundprojekt:

1. Generera ett nytt SSH-nyckelpar direkt i **1Password**:
   - Välj valvet **"Deploy keys"**.
   - Skapa ett nytt objekt med namnet **"PROJECT NAME GitHub Deploy ssh key"** (ersätt PROJECT NAME med projektets namn).
   - Välj att generera en ny SSH-nyckel av typen `Ed25519`.

2. Lägg till **den publika nyckeln** på servern:

   - Kopiera den publika nyckeln från 1Password-objektet.
   - Klistra in den i `~/.ssh/authorized_keys` för användaren `SSH_USER` på servern.

3. Lägg in **den privata nyckeln** som `SSH_KEY` i GitHub:

   - Kopiera den privata nyckeln från 1Password-objektet (inklusive `-----BEGIN` / `-----END`).
